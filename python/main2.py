...
def get_missing(config:Config):
    db = DB(config.db_path)
    db_payslips_filenames = db.fetch("SELECT FileName FROM Bulletins")
    db.close()
    # Sqlite gives out a list of tuples, so we flatten it
    db_payslips_filenames = [item for subtuple in db_payslips_filenames for item in subtuple]
    mail_retriever = MailRetriever(
        ...
        config.mail_subject,
        config.debug)
    mail_retriever.download_missing_payslips(config.pdf_folder, db_payslips_filenames)
